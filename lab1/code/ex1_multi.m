...
% Прогноз
house = [1650, 3];
house_normalized = (house - mu) ./ sigma;
house_normalized = [1, house_normalized];
price = house_normalized * theta;
...
house = [1, 1650, 3];
price = house * theta;
% нормализация признаков только для градиентного спуска, а не для нормального
% уравнения. В градиентном спуске мы нормализуем признаки для обеспечения их
% схожей шкалы и более эффективной сходимости алгоритма.
% в нормальном уравнении нормализация не требуется, так как
% оно предоставляет аналитическое решение без необходимости итераций.
